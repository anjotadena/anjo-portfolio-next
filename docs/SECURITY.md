...
- **Secrets**: `.env*` is git-ignored and Docker-ignored; the image contains no secrets; `/api/health` reports modes and counts only; error responses never include messages or stacks; logs drop fields named `message`, `email`, `body`, `content`, `query`, `prompt`, `answer`, `apiKey`, `token`, `secret`, `databaseUrl`, etc.
